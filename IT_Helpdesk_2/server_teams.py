...
def create_app(env_json_file,
    ...
    env_config = EnvConfig(env_json_file)
    ...
    _agent = [Agent(domain, ensemble, _interpreter, _tracker_store,env_config)]
    ...
    def continue_predicting():
        ...
        try:
            ...
            usr_channel = HttpInputChannel(int(env_json['userchannelport2']),'/webhooks/botframework',input_channel)
            ha_channel = None
        except OSError as e:
            logger.error(str(e))
            return str(e)
        ...
        logger.info(f"Starting http server at port {int(env_json['userchannelport2'])}")
        op_agent = agent()
        op_agent.handle_custom_processor(usr_channel,ha_channel,processor)
    ...
    def email(sender_id):
        ...
        teams_channel.output_channel.id_map.update(env_config.inter_channel_mapper)
        #temporary code follows
        teams_channel.output_channel.id_map.update({sender_id:
                            env_config.inter_channel_mapper[list(env_config.inter_channel_mapper.keys())[0]]})
        teams_channel.output_channel.reverse_id_map.update({list(env_config.inter_channel_mapper.keys())[0]:sender_id})
        #temporary code ends
